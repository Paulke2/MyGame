...
public class TextView {
    ...
    public static char getUsersNextActionType(Scanner scr) {
        /**
         * Asks user to input the character a, r, s,k, or m, ignoring case
         *
         * @param scr Scanner class, therfore user can type the string into tha console until they
         *            input a desired value
         * @return the first char that is either a, r, s,k, or m, ignoring case
         */
        boolean validLetter = false;
        String attackType = " ";
        while (!validLetter) {
            System.out.println("Enter either A for Attack, M for Move, R for Recruit, or S for Spawn or K for a karen");
            if (scr.hasNext("K")||scr.hasNext("A")||scr.hasNext("M")||scr.hasNext("R")||scr.hasNext("s")||scr.hasNext("a")||scr.hasNext("m")||scr.hasNext("r")||scr.hasNext("S")) {
                attackType = scr.next();
                validLetter = true;
            } else {
    ...
    public void getNextPlayersAction(GameS22 game) {
        ...
        this.fromSquareRow = getValidInt(0, game.getBoard().getNumRows(), scr);
        this.fromSquareCol = getValidInt(0, game.getBoard().getNumColumns(), scr);
        this.toSquareRow = getValidInt(0, game.getBoard().getNumRows(), scr);
        this.toSquareCol = getValidInt(0, game.getBoard().getNumColumns(), scr);
    }
